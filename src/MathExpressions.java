...
// Метод получает на вход строку с мат. выражением, включающим целые и дробные числа, скобки (), пробелы,
// знак отрицания -, возведение в степень ^, sin(x), cos(x), tan(x)
//Для sin(x), cos(x), tan(x) выражение внутри скобок считаются градусами, например, cos(3 + 19*3)=0.5
//Степень задается так: a^(1+3) и так a^4, что эквивалентно a*a*a*a.
// При помощи рекурсии метод производит все высисления в стандартном для этого порядке

    public static void recurse(final String expression, int countOperation) {
